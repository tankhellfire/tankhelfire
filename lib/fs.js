...
class EteLayer{
  constructor(name,read,write,remove){
    this.name=name
  ...
  async setup(metadataPath=['.etewaott']) {
    ...
    if (metadataPath) {
      this.metadataPath = metadataPath
      try {
        const metadata = JSON.parse(await this.methods.read({path:this.metadataPath}))
    ...
    this.api = {
      ...
      get(paths,threads=3) {
        return paths.map(e=>fs.metaTree(e).d)
      },
      async read(paths,threads=3) {
        let out=new Array(paths.length)
        await thread(paths.map((path,i)=>[e,i]),async(e)=>out[e.i]=await fs.methods.read(fs.metaTree(e.path).d),threads)
        return out
      },

      set(bits) {
        for(const {path,data} of bits){
          fs.metaTree.get(path).d=data
        }
      },

      async write(bits,threads=3) {
        return thread(bits,e=>fs.methods.write(fs.metaTree(e.path).d,e.data),threads)
      },

      async remove(path,threads=3) {
        return thread(paths,async(path)=>{
          await fs.methods.remove(fs.metaTree.get(path).d)
