...
class File{
  ...
  getMetadata(){
    ...
    if(this.root?.metadata){
      return this.metadata=(this.root.metadata.find(e=>e.local.path.join('/')===this.path.join('/'))??this.root.metadata[this.root.metadata.push({local:{path:this.path}})-1])
    }
  }
  ...
  async updateInfo(fileGet,opt={modified:1}){
    ...
    if(this.metadata){
      if(opt.modified&&this.metadata?.local?.modified<out.local.modified){
        out.public.modified=out.local.modified
        console.warn(`updated ${this.path.join('/')}
${JSON.stringify(out)}`)
      }
      return deepAssign(this.metadata,out)
    }
    return out
  }
...
class Fs extends Folder{//showDirectoryPicker
  ...
  async setup(metadataPath=['.etewaott']){
    ...
    try{
      ...
      this.cache={}
      await thread(this.metadata,async(job,jobs)=>{
  ...
  isMeta(arr){
    if(arr.length<this.metadataPath.length)return 0
    return this.metadataPath.every((e,i)=>e===arr[i])
  }

  
  async blindPush(target,opt={threads:10}){
    return thread((await this.refreshAll()).filter(e=>e.constructor==Fs.File&&!this.isMeta(e.path)).sort((a,b)=>(a.path.join('/')>b.path.join('/'))*2-1),async(job,jobs)=>{
      let tFile=await target.get(job.path)
      await tFile.write(await job.read(),{modified:0})
      deepAssign(tFile.metadata.public,job.metadata.public)
    },opt.threads)
  }
  
}
// let a=await fs[0].fs.refreshAll()
// let b=await Promise.all(a.filter(e=>e.constructor==Fs.File&&!fs[0].fs.isMeta(e.path)).sort((a,b)=>(a.path.join('/')>b.path.join('/'))*2-1).map(e=>e.getInfo()))
