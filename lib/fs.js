...
class Cfs{
  ...
  async setup(name=this.folder?.path??'/cfs'){
    ...
    let metadata=[]
    try{
      metadata=JSON.parse(await(await this.folder.get(['0']).read()).text())
    ...
    for(let data of metadata){
      this.fileTree.get(data.loc.path).d=new ChainNode(data).insertAfter(this.metadata.findBefore(node=>data?.loc?.file?.e<node.d?.loc?.file?.s))
    }
  }
  ...
  async set(bits){
    for(let bit of bits){
      this.fileTree.get(bit.path).d.d.pub=bit.data
    }
  }
  ...
  async remove(paths){
    for(let path of paths){
      ...
      a.d.n.p=a.d.p//remove node
      delete a.d//remove from tree
    }
  }
}
