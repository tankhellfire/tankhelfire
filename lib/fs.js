class Folder{
  ...
  async getFolder(name,handle){
    if(typeof name==='string')return this.cache.folders[name]??(this.cache.folders[name]=new Fs.Folder(handle??(await this.handle.getDirectoryHandle(name,{create:1})),[...this.path,name]))
    if(name.length==1)return this.getFolder(name[0])
    return (await this.getFolder(name.shift())).getFolder(name)
  }
  async get(name,handle){
    if(typeof name==='string')return this.cache.files[name]??(this.cache.files[name]=new Fs.File(handle??(await this.handle.getFileHandle(name,{create:1})),[...this.path,name]))
...
class Fs extends Folder{
  ...
  async setup(metadateFolder){
    let perm=this.requestPermission()

    if(!metadateFolder?.get){
      metadateFolder=await this.getFolder('.etewaott')
    }
