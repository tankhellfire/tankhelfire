  ...
  async setup(metadataPath=['.etewaott'],writable=1){
    ...
    this.api={
      ...
      async remove(paths){if(!fs.writable)throw `${fs.name} isn't writable`
        ...
        let files=paths.map(e=>fs.get(e).metadata)
        fs.metadata=fs.metadata.filter(e=>!files.includes[e])
...
// let paths=Array.from(new Set([...a.newFiles,...a.updatedFiles]))
// await fs[0].fs.api.write(
//   (await D.fs.api.read(paths)).map((data,i)=>({
//     path:paths[i],
//     data
//   }))
// )
// await fs[0].fs.api.set(
//   (await D.fs.api.get(paths)).map((data,i)=>({
//     path:paths[i],
//     data
//   }))
// )
