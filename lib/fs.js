...
//console.log(a.newFiles.map(e=>e.local.path),a.updatedFiles.map(e=>e.local.path))
...
class Cfs{
  ...
  async setup(){
    let metadata=[]
    try{
      this.metadata=JSON.parse(await(await this.folder.get(['0']).read()).text())
    }catch(err){console.error('no meta found'),err}


    this.metadata=new ChainNode
    this.fileTree=new TreeNode
    for(let data of metadata)this.fileTree.get(node.d.l.path).d=new ChainNode(data).insertAfter(this.metadata)
  }

  async get(paths){
    return paths.map(path=>this.fileTree.get(path).d.d.p)
  }
  async set(bits){
    for(let bit of bits){
      this.fileTree.get(bit.path).d.d.p=bit.data
    }
  }
