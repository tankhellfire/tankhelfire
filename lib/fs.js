  ...
  get metadata(){
    ...
    if(this.root?.metadata){
      if(this.metadata=(this.root.metadata.find(e=>e.local.path.join('/')===this.path.join('/')))){return this._metadata}else{
        console.warn(`making ${this.path.join('/')}`,this.root)
...
class Fs extends Folder{//showDirectoryPicker
  ...
  async push(target,opt={}){
    let thisFiles=(await this.refreshAll()).filter(e=>e.constructor==Fs.File&&!this.isMeta(e.path))
    let targFiles=(await target.refreshAll({getInfo:{modified:0}})).filter(e=>e.constructor==Fs.File&&!target.isMeta(e.path))
  ...
  async updateFromLocal({refreshOpt={}}={}){
    ...
    return {files,newFiles,unchangedFiles,removedFiles}
  }
...
// let a=await D.fs.updateFromLocal()
// D.fs.metadata=a.files

// a.newFiles.filter(e=>Fs.startsWith(e.local.path,['dmusic'])).forEach(e=>Fs.deepAssign(e.public,{tankhellfire:{music:1}}))
