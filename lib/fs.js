...
class Cfs{
  constructor(folder,size=1024*1024*1){
    ...
    let cfs=this
    this.api={
      async files(){return cfs.metadata.arrayAfter().map(e=>e.loc.path)},
      get:this.get,
      set:this.set,
      read:this.read,
      write:this.write,
  ...
  async rewrite(bits,readFn=null){
    ...
    for(const {path,size} of bits){
      ...
      while(1){
        ...
        break}
        end=(node.n.d?.loc?.s??start);

        if(size<=end-start)break
    ...
    const files=this.metadata.findAll(n=>n.d[invalid])
    for(const file of files){
      const path=file.d.loc.path
      const start=file.d.loc.s
      const end=file.d.loc.e

      const data=readFn([path]).then(e=>e[0])
        
      const chunk=Math.floor(start/chunkSize)
      const chunkStart=chunk*chunkSize
      const chunkEnd=chunkStart+chunkSize
      const blobParts=[]
      let chunkHead=chunkStart
      if(chunkHead<start){
        const readSize=start-chunkHead
        blobParts.push(this.folder.get(['chunks',String(chunk)]).read().then(blob=>blob.slice(0,readSize)))
        chunkHead+=readSize
      }
      const readSize=start-chunkHead
      blobParts.push(data.then(blob=>blob.slice(chunkHead-start,chunkEnd-start)))
    }
  }
