...
class Fs extends Folder{//showDirectoryPicker
  ...
  async setup(metadataPath=['.etewaott']){
    ...
    try{
      this.metadata=JSON.parse((
        await Promise.all(
          (await (await this.getFolder(this.metadataPath)).refresh()
  ...
  async saveMetadata(){
    (await (await fs[0].fs.getFolder(this.metadataPath)).get('m.txt')).write(JSON.stringify(this.metadata))
  }
  
}
// let a=await fs[0].fs.refreshAll()
// let b=await Promise.all(a.filter(e=>e.constructor==Fs.File).map(e=>e.getInfo()))
