class Folder{
  ...
  async refresh(progressFunc=e=>e){
    this.children={}
    this.files={}
    this.folders={}
    ...
    while(1){
      ...
      if(value.kind==="directory"){
        await this.getFolder(value.name,value,progressFunc)
        continue
      }
      if(value.kind==="file"){
  ...
  async getFolder(name,handle,progressFunc=e=>e){
    if(typeof name==='string')return this.folders[name]??(this.folders[name]=this.children[name]=await new Fs.Folder(handle??(await this.handle.getDirectoryHandle(name,{create:1})),[...this.path,name]).refresh(progressFunc))
...
class Fs extends Folder{
  ...
  async setup(progressFunc=e=>e){
    await Promise.all([this.refresh(progressFunc),this.requestPermission()])
