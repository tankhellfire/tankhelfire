...
class Fs extends Folder{//showDirectoryPicker
  ...
  async setup(metadataPath=['.etewaott']){
    ...
    try{
      ...
      this.cache={}
      await Fs.thread(this.metadata,async(job,jobs)=>{
  ...
  async blindPush(target,opt){
    return Fs.thread((await this.refreshAll()).filter(e=>e.constructor==Fs.File&&!this.isMeta(e.path)).sort((a,b)=>(a.path.join('/')>b.path.join('/'))*2-1),async(job,jobs)=>{
...
// fs[0].fs.metadata.filter(e=>Fs.startsWith(e.local.path,['dmusic'])).forEach(e=>console.log(e.public))
