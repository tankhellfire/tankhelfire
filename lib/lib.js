//await Object.getPrototypeOf(async()=>0).constructor('exports',await(await fetch('https://tankhellfire.glitch.me/lib/lib.js')).text()+'\n;return exports')()
...
const lib=async(...paths)=>{
  if(paths.length===1){
    ...
    const path=lib.dir+paths[0].replace(/\/\.\.\//g, '//')
    if(lib.cache[path]===undefined){
      console.log(`loading "${paths[0]}" from ${path}`)
      try{
        lib.cache[path]=await Object.getPrototypeOf(async()=>{}).constructor('exports,lib',await((await(fetch(path))).text())+';\nreturn exports')({},lib)
      }catch(err){
        console.error(err)
        return
      }
