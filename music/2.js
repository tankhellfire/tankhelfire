...
function loadMusic(file) {
    ...
    onSuccess: async (tag) => {
      ...
      if (picture) {
        ...
        mediaSession.artwork = [
          {
            src:
              (icon.href = thumbnail.src =
                await scalePNG(picture.data, picture.format, screen.height)),
          },
        ];
      }
    },
  ...
  seekBar.addEventListener(
    "input",
    (e) => (audioPlayer.currentTime = seekBar.value),
  );
}
...
volume.addEventListener(
  ...
  (setVolume = (e) =>
    (volDisplay.textContent = (
      (100 * (audioPlayer.volume = volume.value)).toFixed(0).padStart("2", 0) +
      "%"
    ).substring(0, 3))),
);
...
sortByTimeButton.onclick = (e) => {
  updateSongPlayTime();
  let a = songList[songNum];
  songNum = KnuthShuffle(songList).map(e=>[e,Number(e?.public?.created??0)])
    .sort(
      (a, b) =>a[1]-b[1]
    ).map(e=>e[0])
    .indexOf(a);
...
speed.addEventListener(
  "change",
  (e) => (audioPlayer.playbackRate = Number(speed.value)),
);
